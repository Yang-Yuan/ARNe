...
class Multi_Head_Attention(nn.Module):
	def __init__(self, d_v, d_k, d_q, d_model, h, N_seq_1, dropout_dot_product=0.1, dropout_fc=0.1, N_seq_2=None):
		...
		self.linear_q = nn.Linear(d_model, h * d_k)
	...
	def scaled_dot_product(self, v, k, q):
		...
		if self.collect_attention_maps:
			...
			self.attention[str(self.i_iteration)] = att.reshape(self.h, self.batch_size, self.N, self.N_2).transpose(0, 1).squeeze(0    )
...
class Encoder_Layer(nn.Module):
	def __init__(self, d_v, d_k, d_q, d_model, h, d_hidden_position_ff, N, dropout_dot_product=0.1, dropout_fc=0.1, dropout_pos_ff=0.1, old_transformer_model=False):
		super().__init__()
		...
		self.multi_head_attention = Multi_Head_Attention(d_v, d_k, d_q, d_model, h, N, dropout_dot_product=dropout_dot_product, dropout_fc=dropout_fc)
...
class Encoder(nn.Module):
	def __init__(self, N, d_model=None, n_layers=None, h=None, d_ff=None, d_v=None, d_k=None, d_q=None, dropout_dot_product=0.1, dropout_fc=0.1, dropout_pos_ff=0.1, old_transformer_model=False):
		"""
		...
		assert (d_v is not None and d_k is not None and d_q is not None) or (d_v is None and d_k is None and d_q is None)
		...
		self.encoder_layers = nn.Sequential(*[Encoder_Layer(d_v, d_k, d_q, d_model, h, d_ff, N, dropout_dot_product=dropout_dot_product, dropout_fc=dropout_fc, dropout_pos_ff=dropout_pos_ff, old_transformer_model=old_transformer_model) for i in range(n_layers)])
