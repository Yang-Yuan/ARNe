...
def main():
    ...
    for mode in ["train", "val", "test"]:
        ...
        for i in tqdm(range(1, size[mode] + 1), desc = mode):
            ...
            assert (not np.min(image) < 0 and not np.max(image) > 255) and (
                        not np.min(target) < 0 and not np.max(target) > 255) and (
                               not np.min(meta_target) < 0 and not np.max(meta_target) > 255)
            ...
            torch.save(torch.from_numpy(meta_target.astype(np.uint8)),
                       pt_file.format("meta_target", dataset_type, mode, i))
